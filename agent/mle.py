...
class TransitionBuffer(object):
    ...
    # def to_buffer(memory):
        
    #     for tt in range(1, len(memory.actions)-1):
...
class Memory:
    ...
    def delete_oldest_trj(self):
        ptr = self.ptr.pop(0)
        del self.actions[:ptr]
        del self.states[:ptr]
        del self.logprobs[:ptr]
        del self.rewards[:ptr]
        del self.obj[:ptr]
        del self.context[:ptr]
        del self.context2[:ptr]
        del self.timestep[:ptr]
